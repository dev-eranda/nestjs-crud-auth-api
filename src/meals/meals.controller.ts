...
export class MealsController {
    ...
    @Get(':id')
    getMealById(@GetUser('sub') userId: number, @Param('id', ParseIntPipe) mealId: number) {
        return this.mealService.getMealById(userId, mealId)
    }

    @Patch(':id')
    editMealById(@GetUser('sub') userId: number, @Param('id', ParseIntPipe) mealId: number, @Body() dto: EditMealsDto) {
        return this.mealService.editMealById(userId, mealId, dto)
    }

    @HttpCode(HttpStatus.NO_CONTENT)
    @Delete(':id')
    deleteMealById(@GetUser('sub') userId: number, @Param('id', ParseIntPipe) mealId: number) {
